...
func reset_col(col_index):
	for i in range(1,6):
		...
		#swap in grid
		var temp = grid[col_index][i]
		grid[col_index][i] = grid[col_index][index]
		grid[col_index][index] = temp
		# if you drag the top tile around twice, it stops looping and will crash
		# if you drag from the bottom around twice, multiple go to 0 I think
		# maybe as soon as one gets out of bounds, I should shift all of them		
